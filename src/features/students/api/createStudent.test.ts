...
vi.mock('./inviteParent', () => ({
  linkParent: () => {
    ...
    return Promise.resolve({ profileId: 'p-1', token: null, emailed: true, emailError: null })
  },
}))
...
describe('createStudent', () => {
  ...
  it('creates, enrols and links in order when everything works', async () => {
    const { studentId, invite } = await createStudent({ academyId: 'ac-1', form, photoFile: null })
    expect(studentId).toBe('stu-1')
    expect(invite).toEqual({ profileId: 'p-1', token: null, emailed: true, emailError: null })
